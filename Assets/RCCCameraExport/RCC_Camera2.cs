...
public class RCC_Camera2 : MonoBehaviour{
	...
	private RCC_HoodCamera2 hoodCam;					// Hood camera. It's a null script. Just used for finding hood camera parented to our player vehicle.
	private RCC_WheelCamera2 wheelCam;				// Wheel camera. It's a null script. Just used for finding wheel camera parented to our player vehicle.
//	private RCC_FixedCamera fixedCam;					// Fixed Camera System.
	...
	void GetTarget(){
		...
		hoodCam = playerCar.GetComponentInChildren<RCC_HoodCamera2>();
		wheelCam = playerCar.GetComponentInChildren<RCC_WheelCamera2>();
...
//		transform.rotation = playerCar.transform.rotation * Quaternion.AngleAxis(10f, Vector3.right);
	...
	public void SetTarget(GameObject player){
        ...
        playerCar = player.GetComponent<RCCData2>();//GameObject.FindGameObjectWithTag("Player").transform;
        GetTarget ();
...
//		}
	...
	bool Occluding(Vector3 targetFollow){
		...
		//linecast from your player (targetFollow) to your cameras mask (camMask) to find collisions.
		if (Physics.Linecast (targetFollow, transform.position, out wallHit, ~(1 << LayerMask.NameToLayer(playerLayer)))) {
	...
	void OnDisable(){
		...
		//.OnRCCPlayerCollision -= RCC_CarControllerV3_OnRCCPlayerCollision;
