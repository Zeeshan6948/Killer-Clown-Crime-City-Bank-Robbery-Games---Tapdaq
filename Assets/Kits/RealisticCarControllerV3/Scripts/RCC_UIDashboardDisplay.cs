...
//		StartCoroutine(LateDisplay());
    ...
    public void GetinOfCar()
    {
        print("clicknig");
        play = LevelManager.m_instance.m_Convertor.PlayerObjects[0].GetComponent<FPSPlayer>();
        play.objToPickup.SendMessageUpwards("PickUpItem", play.gameObject, SendMessageOptions.DontRequireReceiver);
        play.GetComponent<FPSPlayer>().usePressTime = 0.9f;
    }


    public void ComeOutOfCar()
    {
        LevelManager.m_instance.m_Convertor.Convert = true;
        if(LevelManager.m_instance.currentCar.GetComponent<JokerAnimControl>())
            LevelManager.m_instance.currentCar.GetComponent<JokerAnimControl>().StopDriving();
    }

    public void ShootInCar()
    {
        LevelManager.m_instance.currentCar.GetComponent<JokerAnimControl>().StartShooting();
        StartShooting();
    }
