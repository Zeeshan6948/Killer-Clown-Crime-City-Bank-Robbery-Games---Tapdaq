...
public class RCC_CarControllerV3 : MonoBehaviour
{
    ...
    internal float _brakeInput
    {
        get
        {
            ...
            if (!automaticGear || semiAutomaticGear)
            {
                //return Mathf.Clamp01(brakeInputR);
            ...
            else
            {
                if (!cutGas)
                {
                    //return (direction == 1 ? Mathf.Clamp01(brakeInputR) : Mathf.Clamp01(gasInput));
    ...
    private float NoSRegenerateTime = 0f;
    ...
    void GearBox()
    {
        ...
        if (!externalController)
        {
            if (brakeInput > .9f && transform.InverseTransformDirection(rigid.velocity).z < 3f && canGoReverseNow && automaticGear && !semiAutomaticGear && !changingGear && direction != -1)
            {
