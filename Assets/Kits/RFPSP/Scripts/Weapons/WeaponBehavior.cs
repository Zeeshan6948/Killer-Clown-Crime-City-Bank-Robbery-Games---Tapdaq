	...
	void HitObject ( RaycastHit hit, Vector3 directionArg, bool isSecondCast = false ){
		...
		case 0://hit object
			...
			if(hit.collider.gameObject.GetComponent<BreakableObject>()){
				...
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker
			}else if (hit.collider.gameObject.GetComponent<PedestrianController>())
            {
                hit.collider.gameObject.GetComponent<PedestrianController>().ApplyDamage();
                FPSPlayerComponent.UpdateHitTime();//used for hitmarker
            }
                break;
		...
		case 13://hit object is an NPC
            ...
            if(hit.collider.gameObject.GetComponent<CharacterController>()){
				hit.collider.gameObject.GetComponent<CharacterController>().ApplyDamage();
				FPSPlayerComponent.UpdateHitTime();//used for hitmarker
			}
			break;
        case 15://hit object is an TrafficAI
                if (hit.collider.gameObject.GetComponent<AIDamage>())
                {
                    hit.collider.gameObject.GetComponent<AIDamage>().ApplyDamage();
                    FPSPlayerComponent.UpdateHitTime();//used for hitmarker
                }
                break;
            default:
